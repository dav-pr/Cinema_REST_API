...
class UserModelSerializer(serializers.ModelSerializer):
    ...
    - read_only_fields: перелік полів моделі користувача, які можна лише читати (наприклад, ідентифікатор та баланс користувача).

    Методи:
    - create(validated_data): створення нового користувача на основі переданих даних. Пароль кодується перед збереженням у базі даних.
    """
    ...
    - write_only_fields: перелік полів моделі користувача, які можна лише записати та які не будуть відображатися в серіалізованому представленні (наприклад, пароль).
    - read_only_fields: перелік полів моделі користувача, які можна лише читати та які будуть відображатися в серіалізованому представленні (наприклад, ідентифікатор та баланс користувача).
    """
        model = UserModel
